...
resource "azurerm_private_endpoint" "storage-endpoint1" {
  name                = "stc1-endpoint"
  location            = var.location
  resource_group_name = var.resource_group_name
  subnet_id           = azurerm_subnet.data_subnet.id
...
resource "azurerm_private_endpoint" "storage-endpoint2" {
  ...
  private_service_connection {
    name                           = "storage-pc2"
    private_connection_resource_id = var.stac_webapp_id
    subresource_names              = ["file"]
    is_manual_connection           = false
  }
...
resource "azurerm_private_dns_zone_virtual_network_link" "st1-dns-zone-to-vnet-link" {
  name                  = "st1-vnet-link"
  resource_group_name   = var.resource_group_name
  private_dns_zone_name = azurerm_private_dns_zone.pv_st1_dns.name
  virtual_network_id    = azurerm_virtual_network.Vnet.id
}
...
resource "azurerm_private_dns_a_record" "st1filerecord" {
  ...
  resource_group_name = var.resource_group_name
  ttl                 = 300
  records             = [azurerm_private_endpoint.storage-endpoint1.private_service_connection[0].private_ip_address]
}
...
resource "azurerm_private_dns_a_record" "st2filerecord" {
  ...
  resource_group_name = var.resource_group_name
  ttl                 = 300
  records             = [azurerm_private_endpoint.storage-endpoint2.private_service_connection[0].private_ip_address]
}
